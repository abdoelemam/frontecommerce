...
export default function AddProductPage() {
  ...
  const addColorGroup = (name: string, value: string) => {
    ...
    setColorGroups([
      ...colorGroups,
      {
        ...
        colorValue: value,
        files: [],
        sizes: [{ size: "M", stock: 10 }], // Default size
        customSizeInput: "",
      }
    ]);
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(`Product "${name}" has been successfully added to the catalog.`);
      router.push("/admin/products");
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || "Failed to create product. Ensure all details are correct.");
    }
  });
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (fileIndex === 0) {
      toast.error("Please select at least one image for your variants.");
      return;
    }
    ...
    <form onSubmit={handleSave} className="min-h-screen text-[14px] font-body text-on-surface animate-fade-in-up pb-20">
      ...
      <div className="flex items-center justify-between mb-xl">
        <div>
          ...
          <h2 className="font-display text-headline-lg text-on-surface leading-tight">Add New Product</h2>
        </div>
                  ...
                  <div key={cg.id} className="border border-outline-variant/60 rounded-xl overflow-hidden shadow-sm bg-surface">
                    ...
                    <div className="p-lg grid grid-cols-1 md:grid-cols-2 gap-lg">
                      ...
                      <div className="flex flex-col gap-xs">
                        ...
                        <label className="border-2 border-dashed border-outline-variant/60 rounded-lg p-xl flex flex-col items-center justify-center bg-surface hover:bg-surface-container-low transition-colors duration-300 cursor-pointer min-h-[160px]">
                          <input
                            type="file"
                            multiple
                            accept="image/*"
                          ...
                          <p className="font-semibold text-[12px] text-center px-4">
                            {cg.files.length > 0 ? `${cg.files.length} images selected` : "Click to upload images"}
                          </p>
                        </label>
                      </div>
